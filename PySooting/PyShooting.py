...
def game_loop():
    ...
    while not done:
        for event in pygame.event.get():
            if event.type == pygame.KEYDOWN:    # 키 입력 시
                if event.key == pygame.K_LEFT:
                    ...
                    missile = Missile(fighter.rect.centerx, fighter.rect.y, 10)     # speed : 10
                    missile.launch()
        ...
        max_rock_speed = 1 + int(shot_count / 50)  # 운석 50개 당 speed up
...
def game_menu():
    ...
    draw_text('♥또동이♥ 슈팅게임!', font_50, screen, draw_x, draw_y, BLACK)
    draw_text('엔터 키를 누르면', font_30, screen, draw_x, draw_y+200, RED)
    draw_text('게임이 시작돼용♥', font_30, screen, draw_x, draw_y+250, RED)
    pygame.display.update()
